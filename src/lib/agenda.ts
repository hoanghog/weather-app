...
class AgendaClass {
  ...
  async initialize() {
    ...
    await this.agenda.every('7 0 * * *', 'generateForecastBratislava');
    await this.agenda.every('15 0 * * *', 'generateForecastMunich');
    await this.agenda.every('22 0 * * *', 'generateForecastParis');
    await this.agenda.every('30 0 * * *', 'generateForecastWarsaw');
    await this.agenda.every('37 0 * * *', 'generateForecastPrague');
  }
